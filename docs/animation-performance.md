...
Canvas closing uses the same 500ms fade, 24px blur, and easing as the front-door content. Only the content panel dissolves; its tab stays selected until the fade completes, then disappears immediately as the neighboring canvas becomes visible. The Web Animations API keeps the completed fade in place through the content replacement, with no slide or scale. Reduced motion closes immediately. The sidebar also uses this API so it can reverse immediately without introducing another snapshot lifecycle alongside React's existing surface transitions. [Web Animations API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Animations_API/Using_the_Web_Animations_API).
