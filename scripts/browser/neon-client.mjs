...
import { href, canvas, target } from './fixtures.mjs';
...
try {
    ...
    const other = '/build?destination=' + encodeURIComponent(JSON.stringify({ version: 1, owner: 'kf', surface: 'build', target, canvas }));
    await p.goto(origin + other);
    await input.waitFor();
