...
try {
  for (const motion of ['reduce', 'no-preference']) {
    ...
    const rowGeometry = node => [...node.querySelectorAll('[data-today-row]')].map(row => ({ order: row.dataset.todayRow, width: row.offsetWidth, height: row.offsetHeight }));
    const liveToday = page.locator('fieldset[aria-label="Today"]');
    assert.deepEqual(await readOnlyToday.evaluate(rowGeometry), await liveToday.evaluate(rowGeometry), 'At the same width, read-only Today retains the exact row/card geometry');
    ...
    assert.equal(await page.getByRole('tab', { name: 'Lead routing → UAT', exact: true }).count(), 0);
    await selectProject(page, 'feature/lead-routing');
