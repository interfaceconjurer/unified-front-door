...
export function DayZeroHome({ snapshot, onExplore }: { snapshot?: AssessmentState; onExplore?: (surface: SurfaceApp) => void }) {
  ...
  return <div className={styles.home}>
    ...
    <div className={styles.connection}>
      ...
      <button className={styles.textButton} type="button" aria-expanded={scopeOpen} aria-controls={`${id}-scope`} onClick={() => { setScope(state.scopeOrgIds); setScopeOpen(!scopeOpen); }}>
    ...
    {state.draft ? <ProjectReview headingId={`${id}-review`} draft={state.draft} findings={available} onChange={store.saveDraft} onBack={() => store.saveDraft(null)} onCreate={() => {
      ...
      {complete && <section className={styles.findings} aria-labelledby={`${id}-opportunities`}>
    ...
    <div className={styles.explore}><p>You can also explore your workspace while your agent gets to know it.</p><SurfaceNav onExplore={onExplore} /></div>
  </div>;
}
