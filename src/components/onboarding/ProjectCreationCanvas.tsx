...
export function ProjectCreationCanvas({ spec }: { spec: CanvasSpec }) {
  ...
  const router = useRouter();
  const available = findingsForScope(state.scopeOrgIds).filter(
    (finding) =>
      !state.projects.some((project) =>
        project.workItems.some((item) => item.findingId === finding.id),
      ),
  );
  if (!state.draft || state.status !== "complete")
    return (
      <section className={`${styles.projectCanvas} ${styles.review}`}>
        <h1>Start a new project</h1>
        <p>
          Choose opportunities from your completed org assessment to start a
          project.
        </p>
        <button
          type="button"
  ...
  return (
    <ProjectReview
      ...
      draft={state.draft}
      findings={available}
      onChange={store.saveDraft}
      onBack={() => router.push("/", { scroll: false })}
      onCreate={() =>
        createWorkspace(() => {
          const project = store.createProject(
            profile?.name ?? "Sam Patel",
            (project) => {
              conversationStore.dispatch(
                workspaceSessionKey(project.id, "main"),
                {
                  type: "surface",
                  scopeKey: "alm",
                  label: "ALM",
                  reply: projectCreatedReply(project),
                },
              );
            },
          );
...
function ProjectReview({
  ...
  const selected = findings.filter((finding) =>
    draft.findingIds.includes(finding.id),
  );
  return (
    <section
      className={`${styles.projectCanvas} ${styles.review}`}
      aria-labelledby={headingId}
    >
      <button className={styles.textButton} type="button" onClick={onBack}>
        <ChevronLeftIcon width={15} height={15} aria-hidden="true" />
        Back to opportunities
      </button>
      <div className={styles.sectionHead}>
        <div>
          <p className={styles.kicker}>FROM INSIGHT TO ACTION</p>
          <h1 id={headingId} tabIndex={-1}>
            Start a new project
          </h1>
          <p>
            I’ve drafted a starting point. Adjust the scope, then create your
            project.
          </p>
        </div>
      </div>
      ...
      >
        ...
        <div className={styles.formRow}>
          <label className={styles.field}>
            ...
            <select
              ...
              onChange={(event) =>
                onChange({ ...draft, targetOrgId: event.target.value })
              }
            >
              {ASSESSMENT_ORGS.filter(
                (org) =>
                  org.kind === "sandbox" && org.connection === "connected",
              ).map((org) => (
          ...
          <div className={styles.field}>
            Project owner
            <strong className={styles.owner}>{profile?.name}</strong>
          </div>
        </div>
        <h3 className={styles.workHeading}>
          Planned work items <span>{selected.length}</span>
        </h3>
        <p className={styles.quiet}>
          Each item includes its evidence, implementation steps, and validation
          criteria.
        </p>
        <div className={styles.reviewItems}>
          {findings.map((finding) => (
            <div className={styles.reviewItem} key={finding.id}>
              <label>
                <input
                  type="checkbox"
                  checked={draft.findingIds.includes(finding.id)}
                  onChange={(event) =>
                    onChange({
                      ...draft,
                      findingIds: event.target.checked
                        ? [...draft.findingIds, finding.id]
                        : draft.findingIds.filter((id) => id !== finding.id),
                    })
                  }
                />
                <span>
                  ...
                  <small>
                    {finding.priority} priority · {finding.effort} ·{" "}
                    {
                      ASSESSMENT_ORGS.find((org) => org.id === finding.orgId)
                        ?.label
                    }
                  </small>
                </span>
              ...
              {draft.findingIds.includes(finding.id) && (
                <details className={styles.evidence}>
                  <summary>Review work item plan</summary>
                  <ol>
                  ...
                  <p>
                    <strong>Validation:</strong> {finding.validation}
                  </p>
                </details>
              )}
            </div>
        ...
        <div className={styles.selectionBar}>
          <div>
            ...
            <span>
              Create a project with {selected.length} work items, then review
              the plan before starting work.
            </span>
          </div>
          <button
            className={styles.primary}
            type="submit"
            disabled={
              !selected.length || !draft.name.trim() || !draft.goal.trim()
            }
          >
            Create project{" "}
