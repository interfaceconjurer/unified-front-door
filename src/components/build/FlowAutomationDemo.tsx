    ...
    timerRef.current = window.setTimeout(() => {
      dispatch({
        ...
        result: {
          ...
          status: "succeeded",
          userSummary: "Build acknowledged the sample outcome. Edge Communications routes to Enterprise Queue.",
    ...
    <section className={styles.surface} aria-labelledby={`canvas-heading-${canvasId}`}>
      <header className={styles.artifactHeader}>
        <div className={styles.titleGroup}>
          ...
          <div className={styles.titleRow}><span className={styles.titleIcon} aria-hidden="true"><WorkflowIcon width={22} height={22} /></span><div><h1 id={`canvas-heading-${canvasId}`} tabIndex={-1}>Flow · {FLOW_ARTIFACT.name}</h1><p>Routes high-value Leads while keeping the sample draft inspectable in Build.</p></div></div>
        </div>
      ...
      <div className={styles.workArea}>
        <div className={styles.canvas} aria-label="Route High-Value Leads Flow">
          <div className={styles.canvasHeader}><div><h2>Flow path</h2><p>Select an element to inspect Build-owned configuration.</p></div><span className={styles.fixtureBadge}>Prototype fixture</span></div>
        ...
        <aside className={styles.inspector} aria-labelledby="inspector-heading">
          ...
          {selectedNode.id === "high-value" ? <dl className={styles.details}><div><dt>Resource</dt><dd>Lead.AnnualRevenue</dd></div><div><dt>Operator</dt><dd>Greater than or equal</dd></div><div><dt>Value</dt><dd>$250,000</dd></div><div><dt>Yes path</dt><dd>Enterprise Queue</dd></div></dl> : <p className={styles.inspectorHint}>The Agent can reference this selection, but Build remains authoritative.</p>}
          <div className={styles.preview}><div className={styles.previewHeader}><div><p className={styles.inspectorEyebrow}>Sample data</p><h2 id="flow-preview-heading" tabIndex={-1}>{sampleVisible ? "Sample outcome" : "Inspect the routing outcome"}</h2></div>{sampleVisible && <span className={styles.previewCheck} aria-hidden="true"><CheckIcon width={18} height={18} /></span>}</div>
            {sampleVisible ? <dl className={styles.previewResult}><div><dt>Lead</dt><dd>{FLOW_ARTIFACT.preview.record}</dd></div><div><dt>Annual Revenue</dt><dd>{FLOW_ARTIFACT.preview.annualRevenue}</dd></div><div><dt>Path</dt><dd>{FLOW_ARTIFACT.preview.path}</dd></div><div><dt>Outcome</dt><dd>{FLOW_ARTIFACT.preview.outcome}</dd></div></dl> : <p className={styles.previewHint}>Uses one fixture Lead. It does not save, validate, or change Salesforce.</p>}
            <button id="flow-preview-button" type="button" className={styles.previewButton} onClick={showSampleOutcome}>{sampleVisible ? "Show sample outcome again" : "Show sample outcome"}</button>
            <p className={styles.focusHint}>Interactive prototype · sample data · nothing is saved or run in Salesforce.</p>
          </div>
        </aside>
      </div>
    </section>
