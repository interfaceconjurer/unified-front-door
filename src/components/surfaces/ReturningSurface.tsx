...
export function ReturningSurface({ surfaceId, children }: {
  ...
  const { orgs, activeProject, activeOrg, setActiveOrg, openProjectPanel } = useWorkspace();
  ...
  return <div className={styles.surface}>
    <header className={styles.header}>
      ...
      {surfaceId !== "code" && <div className={styles.context}>
        ...
        <button type="button" className={styles.projectLink} onClick={openProjectPanel}
          aria-label={`${activeProject.name}, show in projects panel`} aria-controls="workspace-panel" aria-expanded={panelOpen}>
          {activeProject.name}
        </button>
      </div>}
    </header>
    ...
    {surfaceId === "alm" && activeProject.apps.length > 0 && <section aria-labelledby="deployed-heading">
