...
import { surfaceApps, surfaceAppById, type SurfaceApp } from "./app-catalog";
...
const STARTERS: readonly Starter[] = [
  ...
  },
  {
    ...
    surfaceId: "code",
    capabilityId: "react-app",
    fallbackSurfaceId: "build",
    fallbackCapabilityId: "experience",
    Icon: PuzzleIcon,
...
export function FrontDoor({ snapshot, active, onOpenCanvas, onExplore, onOpenWork }: {
  snapshot: TodaySnapshot;
  active: boolean;
  onOpenCanvas: (surface: SurfaceApp, canvas: CanvasSpecInput, prompt: string) => void;
  ...
  const starters = STARTERS.flatMap((starter) => {
    ...
    if (starter.fallbackSurfaceId && availableSurfaces.some((surface) => surface.id === starter.fallbackSurfaceId)) {
      return [{ ...starter, surfaceId: starter.fallbackSurfaceId, capabilityId: starter.fallbackCapabilityId! }];
    }
    return [];
  });
  ...
  function openStarter(surfaceId: SurfaceApp["id"], capabilityId: string, prompt: string) {
    const capability = capabilityForCanvas(surfaceId, capabilityId);
    if (!capability) return;
    onOpenCanvas(surfaceAppById(surfaceId), { kind: "capability", title: capability.label,
      params: { surface: surfaceId, capability: capabilityId } }, prompt);
  }

  return (
    <div className={styles.frontDoor}>
        <fieldset className={styles.content} disabled={!active}>
          {dayZero ? <DayZeroHome snapshot={active ? undefined : snapshot.assessment} onExplore={onExplore} /> : returning ? <ReturningHome snapshot={snapshot} readOnly={!active} onExplore={onExplore} onOpenWork={onOpenWork} /> : <>
          ...
          <SurfaceNav onExplore={onExplore} revealOrder={2} readOnly={!active} />

          <section className={styles.starters} data-front-door-row="3" aria-labelledby={`${id}-starters`}>
              ...
              {starters.map((starter) => (
                <li key={starter.title}>
                  <button
                    ...
                    onClick={() => openStarter(starter.surfaceId, starter.capabilityId, starter.prompt)}
                  >
                    ...
                    <span className={styles.starterAction} data-today-action>
                      Open in {surfaceAppById(starter.surfaceId).label}
          ...
          {canUseCode && (
            <div className={styles.existingProject} data-front-door-row="4">
              ...
              <button type="button" data-today-action onClick={() => openStarter("code", "sfdx-project", EXISTING_PROJECT_PROMPT)}>
                Bring your project
                <ChevronRightIcon width={15} height={15} aria-hidden="true" />
              </button>
            </div>
