...
async function canvasCommand(client: PoolClient, session: OwnedSession, command: Extract<ApplicationCommand, { kind: "canvas.save" | "canvas.copy" }>): Promise<CommandResult> {
  ...
  if (!profile.surfaceAccess.includes(command.surface)) invalid("This surface is unavailable for the current demo profile.");
  ...
  if (!row) {
    ...
    if (command.canvas.kind === "work" && workForCanvas(command.canvas.params)?.surfaceId !== command.surface) invalid("The work destination is unavailable.");
  ...
  if (row) await client.query("UPDATE canvas_drafts SET fields=$4,revision=$5 WHERE namespace_id=$1 AND profile_id=$2 AND id=$3", [...scope, id, fields, revision]);
  else {
    const inserted = await client.query("INSERT INTO canvas_drafts(namespace_id,profile_id,id,surface_id,canvas,target,fields,revision) VALUES($1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT(namespace_id,profile_id,id) DO NOTHING", [...scope, id, command.surface, command.canvas, command.target, fields, revision]);
