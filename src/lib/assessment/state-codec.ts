...
export function decodeAssessment(value: unknown): Decoded<AssessmentState> {
  ...
  if (legacy) for (const project of projects) if (project && project.runId) runs.push({ id: project.runId, startedAt: null, completedAt: null, scopeOrgIds: [...project.scopeOrgIds], findings: project.workItems.map((item) => structuredClone(item.finding)), source: { adapter: "legacy-browser", version: "1" } });
  ...
  if (projects.some((project) => project?.source !== "brief" && !runs.some((run) => run?.id === project?.runId))) return { error: "invalid" };
